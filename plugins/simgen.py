...
SID = [i + LGW_LON for i in range(0, 10, 2)]
STAR = [LGW_LON - i for i in range(10, -1, -2)]
WPT_NAME = "ACORN"
WPT_MAX = 4000
WPT_MIN = 3000
        ...
        if cls.counter < len(SID):
            for i in range(traf.ntraf):
                ...
                for j in range(len(SID)):
                    ...
                    if cls.counter == 1:
                        stack.stack(f'DEST {acid}, EGKK')

                    stack.stack(f'ECHO Adding next waypoint {cls.SID[j] if cls.sidstar[i] == 0 else cls.STAR[j]} called' +
                                f'{WPT_NAME, cls.SID[j]} to {acid}')
                    if j:            
                        stack.stack(f'{acid} AFTER {WPT_NAME} ADDWPT {WPT_NAME},' + 
                                    f'{cls.SID[j] if cls.sidstar[i] == 0 else cls.STAR[j]}')
                    else:
                        stack.stack(f'ADDWPT {acid}, {WPT_NAME},' + 
                                    f'{cls.SID[j] if cls.sidstar[i] == 0 else cls.STAR[j]}')
