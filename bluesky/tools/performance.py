...
#altitude check
    Aaalt = np.array((alt> (1*ft)) & (alt<=(8000.*ft)))
...
#b. alt<3000ft, Vmcr+10>V>Vmap+10
    Abalt = np.array((alt> (1*ft)) & (alt<=(3000.*ft)))
    if bada==True:
...
# phase GND: alt < 0.001ft
    gd = np.array((alt<=(1*ft)) & ((gs <=30*kts)))*6
...
#hdgsel == True: Aircraft is turning
    noturn = np.array(hdgsel==True)*100
...
# tropopause
    abtp = np.array(alt>11000)
    beltp = np.array(alt<11000)
...
# case b: constant MA below TP (at the moment just ISA: tISA = 1)
   # tISA = (self.temp-self.dtemp)/self.temp
    efb = 1/((1+((gamma*R*beta)/(2*g0))*M**2))*np.logical_and.reduce([cspd, abco, beltp])*1

# case c: constant CAS below TP (at the moment just ISA: tISA = 1)

    efc = 1/(1+(((gamma*R*beta)/(2*g0))*(M**2))+ \
    ((1+gamma1*(M**2))**(-1/(gamma-1)))*(((1+gamma1*(M**2))**gamma2)-1)) *\
    np.logical_and.reduce([cspd, belco, beltp])*1
    
#case d: constant CAS above TP
    efd = 1/(1+((1+gamma1*(M**2))**(-1/(gamma-1)))*(((1+gamma1*(M**2))**gamma2)-1))* \
...
def vmin(vmto, vmic, vmcr, vmap, vmld, phase):
    ...
    vmin = (phase==1)*vmto + (phase==2)*vmic + (phase==3)*vmcr + (phase==4)*vmap + (phase==5)*vmld + (phase==6)*0. 
    return vmin    
#------------------------------------------------------------------------------
#
# LIMITS
...
# set altitude to max. possible altitude if alt>Hmax
    if(hcomp.all() ==0):
        lalt = (hcomp ==0)*(hmaxact-1) + (hcomp!=0)*0.0
      #  print "above max alt"
