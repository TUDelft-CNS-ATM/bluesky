...
inline double _rwgs84(const double& sinlat, const double& coslat)
{
...
inline double _dist(const qdr_d_in& ll1, const qdr_d_in& ll2)
{
    ...
    if (ll1.lat * ll2.lat >= 0.0) {
        r = _rwgs84(sin(0.5 * (ll1.lat + ll2.lat)), cos(0.5 * (ll1.lat + ll2.lat)));
    } else {
        r = 0.5 * ( fabs(ll1.lat) * (_rwgs84(ll1.sinlat, ll1.coslat) + a) + 
                    fabs(ll2.lat) * (_rwgs84(ll2.sinlat, ll2.coslat) + a))
...
inline pos _qdrpos(const double& lat1, const double& lon1, const double& dist, const double& qdr)
{
    double R = _rwgs84(sin(lat1), cos(lat1));
...
inline double _kwikdist(const kwik_in& in)
{
    static const double re = 6371000.0;  // readius earth [m]
    double dangle  = sqrt(in.dlat * in.dlat + in.dlon * in.dlon * in.cavelat * in.cavelat);
    return re * dangle;
}

inline double _kwikqdr(const kwik_in& in)
{
