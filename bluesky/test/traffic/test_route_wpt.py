...
def test_route_get_runway_route(route_):
    route = route_.Route()
    route.wpname = ['LIFFY', 'WOODY', 'GINGA']
    route.nwp = len(route.wpname)
    succ, idx = route.get_runway_route()
    assert not succ

    route.wpname.extend(['EHAM/RW26', 'EHAM/RW8'])
    route.nwp = len(route.wpname)
    succ, idx = route.get_runway_route()
    assert succ
    assert idx == 3
